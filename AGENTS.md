...
- **Do not write long registry descriptions, and do not repeat generic verbs in them** (`build`, `work`, `explain`, `use`). `router.ts` counts description tokens **with repetition** at weight 1 — a word used 3× scores 3 and drags unrelated questions to your asset. This has caused real misroutes ("how much protein…" → openai; "explain how photosynthesis works" → openai). Keep descriptions short and noun-heavy; let tags route.
