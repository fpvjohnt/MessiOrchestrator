...
const ORCHESTRATOR_INSTRUCTIONS = [
  ...
  "KEEP CASES FAST — minimize round-trips (each tool call is a slow hop, especially over the phone bridge): do the whole case in as FEW tool calls as you can — [plan_case only if genuinely complex] -> ONE task_assets batch -> synthesize_case -> close_case — and never make many sequential task_asset calls. When a request has a reasonable default interpretation, pick the most sensible one, state it in a short line, and proceed rather than asking; reserve clarifying questions for genuinely ambiguous requests. (Still confirm before an irreversible or outward-facing action — submitting a form, sending, purchasing — that safety check stays.)",
